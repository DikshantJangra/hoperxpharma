...
import { FiPlus, FiSearch, FiDownload, FiFilter, FiMail } from 'react-icons/fi';
import Link from 'next/link';
import SendEmailModal from '@/components/messages/email/SendEmailModal';
...
const PORowSkeleton = () => (
    <tr className="border-b border-gray-100 animate-pulse">
        ...
        <td className="px-4 py-3"><div className="h-4 bg-gray-200 rounded w-24"></div></td>
        <td className="px-4 py-3"><div className="h-8 bg-gray-200 rounded w-8"></div></td>
    </tr>
);
...
export default function PurchaseOrdersPage() {
    ...
    const handleSendEmail = (order: any, e: React.MouseEvent) => {
        ...
        <>
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3">
                                            <button
                                                ...
                                                className="p-2 text-[#10b981] hover:bg-[#d1fae5] rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                                                title={order.supplier?.email ? `Send email to ${order.supplier.name}` : 'No email available'}
                                            >
            ...
            {selectedPO && (
                <SendEmailModal
                    ...
                    recipientName={selectedPO.supplier?.name || 'Supplier'}
                    subject={`Purchase Order ${selectedPO.poNumber}`}
                    defaultMessage={`Dear ${selectedPO.supplier?.name || 'Supplier'},\n\nRegarding Purchase Order: ${selectedPO.poNumber}\nOrder Date: ${new Date(selectedPO.orderDate).toLocaleDateString()}\nTotal Amount: ₹${Number(selectedPO.total).toLocaleString('en-IN')}\n\nPlease confirm receipt of this purchase order.\n\nBest regards`}
                    contextType="PO"
                    contextId={selectedPO.id}
                />
            )}
        </>
    );
