    ...
    <div className="h-screen flex flex-col bg-[#f8fafc]">
      <div className="bg-white border-b border-[#e2e8f0] px-6 py-4">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 text-sm">
              <div className="px-3 py-1.5 bg-[#fef3c7] rounded-lg">
                ...
                <span className="ml-2 font-semibold text-[#92400e]">12</span>
              </div>
              <div className="px-3 py-1.5 bg-[#fee2e2] rounded-lg">
                <span className="text-[#64748b]">Urgent:</span>
                <span className="ml-2 font-semibold text-[#991b1b]">3</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by Rx ID, patient name, phone, clinician... (Press / to focus)" className="w-full pl-10 pr-4 py-2 border border-[#cbd5e1] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0ea5a3]" />
        </div>
      </div>

      <div className="flex-1 flex overflow-hidden">
        <QueueList filter={filter} searchQuery={searchQuery} onSelectRx={setSelectedRx} selectedRx={selectedRx} />
        <div className="flex-1 overflow-hidden">
          {selectedRx ? <RxDetailPanel rx={selectedRx} /> : (
        ...
        {selectedRx && <ContextDrawer rx={selectedRx} />}
      </div>
    </div>
  );
}
