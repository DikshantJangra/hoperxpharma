...
export interface SerializedMedicine {
  canonicalId: string;
  name: string;
  genericName?: string;
...
export class ExportService {
   ...
   * Requirements: 10.1
   */
  serialize(medicine: any): SerializedMedicine {
    return {
   ...
   * Requirements: 10.4
   */
  deserialize(data: SerializedMedicine): any {
    return {
      id: data.canonicalId,
      name: data.name,
   ...
   * Requirements: 10.4
   */
  prettyPrint(medicine: any): string {
   ...
   * Requirements: 10.2
   */
  async exportChanges(since: Date, options: ExportOptions = {}): Promise<SerializedMedicine[]> {
  ...
  async exportForStore(
    ...
    const merged = await storeOverlayService.getMergedMedicines(storeId, canonicalIds);
   ...
   * Requirements: 10.6
   */
  async exportAll(options: ExportOptions = {}): Promise<SerializedMedicine[]> {
    const { skip = 0, take = 1000, status } = options;
  ...
  async getExportStats(): Promise<any> {
    ...
    const pending = await prisma.medicineMaster.count({
      where: { status: MedicineStatus.PENDING },
    });
   ...
   */
  async exportToCsv(medicines: SerializedMedicine[]): Promise<string> {
  ...
  async *batchExport(
    ...
    options: ExportOptions = {}
  ): AsyncGenerator<SerializedMedicine[], void, unknown> {
